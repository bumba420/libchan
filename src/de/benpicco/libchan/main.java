	...
	public static void main(final String[] args) {
		...
		try {
			...
			InputStream in = new BufferedInputStream(
					new URL(thread).openStream());
			parser.parseThread(in, new SimplePostReceiver(parser));

			// InputStream in = new FileInputStream("/tmp/0");
		...
		for (Image img : post.images) {
			try {
				if ((new File(dir + img.filename)).exists()) {
					System.out.println(dir + img.filename
							+ " already exists, skipping.");
					continue;
				}
				System.out.println("Saving " + img.url + " as " + dir
						+ img.filename);
				URL image = new URL(img.url);
				ReadableByteChannel rbc = Channels
						.newChannel(new BufferedInputStream(image.openStream()));
